    ...
    for (feature_index, feature) in enumerate(all_features):
        ...
        for start_index, end_index, cls_pred, span_mask in \
            ...
            if span_mask:
                ...
                if final_text in example.sent_tokens:
                    cnt = example.sent_tokens.count(final_text)
                ...
                d = final_text + "'s"
                e = final_text +'~'
                if a in " ".join(example.sent_tokens):
                    final_text = a
                if b in " ".join(example.sent_tokens):
                    final_text = b
                if c in " ".join(example.sent_tokens):
                    final_text = c
                if d in " ".join(example.sent_tokens):
                    final_text = d
                if e in " ".join(example.sent_tokens):
                    final_text = e

                # if final_text is not None:
                #    for i in example.term_texts:
                #        if final_text in i and ' of ' in i:
                #            final_text = i
                pred_terms.append(final_text)
                pred_polarities.append(id_to_label[cls_pred])
